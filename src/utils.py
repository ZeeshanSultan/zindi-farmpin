...
def make_sub(probs):
    test_sub = pd.read_csv(
        os.path.join(raw_data_dir, "sample_submission.csv"), index_col=0
    )
...
def mask_raster(shapes, raster, upsample=False):
    """
    Get masks from raster using polygons in shapefile

    :param shapes:
    :param raster: Raster data
    :param upsample: Flag to upsample (x2) data
    :return:
    """
        ...
        for dataset in ["train", "test"]:
            ...
            for label in ["JEP", "JFP"]:
                ...
                img_fpath = os.path.join(
                    interim_data_dir, f"images/2017-01-01/B02_{label}.jp2"
                )
